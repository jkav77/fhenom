...
class CkksVector {
 ...
 public:
  ...
  /**
   * @brief Evaluates the sign in each slot of the vector using the Chebyshev function
   *
   * @return CkksVector A vector with -1 in negative slots, 0 in 0 slots, and 1 in positive slots
   */
   ...
   * @return CkksVector A vector of `1`s in the place of equal slots; `0`s elsewhere
   */
   ...
   *
   * @return CkksVector A vector where slot 1 contains the sum of all values in the vector
   */
   ...
   * data
   */
  inline std::vector<lbcrypto::Ciphertext<lbcrypto::DCRTPoly>> getData() const { return data_; }
   ...
   */
  inline void setData(const std::vector<lbcrypto::Ciphertext<lbcrypto::DCRTPoly>> &data,
                      size_t numElements) {
   ...
   * @param context
   */
  inline void setContext(const fhenom::Context &context) { this->context_ = context; }
  ...
  inline void setNumElements(size_t numElements) { this->numElements_ = numElements; }
};
...
inline fhenom::CkksVector operator-(fhenom::CkksVector lhs, const double &rhs) { return lhs -= rhs; }
