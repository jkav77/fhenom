...
#pragma omp parallel for
    for (unsigned ctxt_index = 0; ctxt_index < num_ctxts; ++ctxt_index) {
        for (unsigned filter_index = 0; filter_index < channels_per_ctxt; ++filter_index) {
...
std::vector<std::vector<double>> CkksTensor::createMaskedConvVectors(const Tensor& kernel,
                                                                     const size_t filter_number) const {
